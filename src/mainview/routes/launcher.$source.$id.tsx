import SaveHistory from '../components/game/SaveHistory';
...
function RouteComponent ()
{
  ...
  useBlocker({ shouldBlockFn: () => savesBusy || (!launchFailed.current && !!data) });
  ...
  return <AnimatedBackground ref={ref} backgroundKey='game-details'>
    <FocusContext value={focusKey}>
      <div className='flex shadow-2xs shadow-black flex-col absolute w-screen h-screen overflow-hidden justify-center items-center gap-4'>
        <DotsLoading />
        {error?.startsWith('Save conflict:') ? <div className="w-[min(36rem,94vw)] max-h-[80vh] overflow-y-auto rounded-3xl bg-base-100 p-4 md:p-6">
          <SaveHistory source={source} id={id} busy={savesBusy} setBusy={setSavesBusy} close={HandleGoBack} conflictOnly />
        </div> : <><h1 className='font-semibold'>{error ? 'Could not launch game' : `Launching ${data?.name ?? 'game'}…`}</h1>
      ...
      <FloatingShortcuts />
      <AutoFocus key={error ? 'failed' : 'launching'} force focus={() => error?.startsWith('Save conflict:') ? setFocus('save-history-close') : focusSelf()} />
      </FocusContext>
  </AnimatedBackground>;
}
