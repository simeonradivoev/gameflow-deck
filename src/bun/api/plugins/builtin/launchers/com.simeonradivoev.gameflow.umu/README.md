...
Save locations come from game-specific source metadata, with Windows paths mapped into the game's Wine prefix. The prefix and its entire users directory are not registered as save slots. Games without save metadata have no known save location; their Wine prefix remains on disk and is not deleted. Existing detail views rediscover locations, and the next launch replaces remembered slots with the current game-specific locations.
