...
- Linux store launch wrappers must account for Steam Input exposing both the Steam Deck's physical controller (`28de:1205`) and the Steam virtual gamepad (`28de:11ff`) to SDL games. When the virtual device is present and Steam has not already supplied an SDL ignore list, hide only the physical Deck device with `SDL_GAMECONTROLLER_IGNORE_DEVICES`; do not apply the filter unconditionally because Desktop Mode without Steam needs the physical controller.
